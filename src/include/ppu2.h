...
class Ppu2 {
   ...
   private:
    ...
    size_t GetNextState(std::array<Ppu_States, 3>& nextActions);

    int GetColorFromPalette(uint8_t palette, uint8_t pixel);

    void OnEnter_PrerenderClear();
    void OnEnter_PrerenderTransferY();

    void OnEnter_RenderSkipOdd();
    void OnEnter_RenderProcessNextTile();
    void OnEnter_RenderIncrementScrollY();
    void OnEnter_RenderLoadShiftersAndTransferX();
    void OnEnter_RenderLoadNextBackgroundTile();
    void OnEnter_RenderDoOAMTransfer();
    void OnEnter_RenderUpdateSprites();
    void OnEnter_RenderEndFrameRendering();

    static void (Ppu2::*m_OnEnterCallbacks[Ppu_States::kStatesSize])();
